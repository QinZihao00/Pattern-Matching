...
def getContours(img):
    ...
    img_erosion = cv2.erode(img_binary_inv, kernel, iterations=2)
    img_opening = cv2.morphologyEx(img_binary_inv, cv2.MORPH_OPEN, kernel)
    img_closing = cv2.morphologyEx(img_binary_inv, cv2.MORPH_CLOSE, kernel)
    # Get Contour
    contours, hierarchy = cv2.findContours(img_dilation, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

    return [tmp for tmp in contours if tmp.shape[0] >= 5]
...
def match(template_FD, sample_FDs):
    ...
    for spFD in sample_FDs:
        ...
        if metrics['cosine similarity'] >= simThreshold and metrics['l1 norm'] <= disThreshold:
